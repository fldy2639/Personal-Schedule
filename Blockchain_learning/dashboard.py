#!/usr/bin/env python3
"""
Week4 Dashboard：USDC Transfer 实时整合展示。
"""
...
def load_realtime_data(
    ...
    """
    加载实时 USDC Transfer 并返回标准化 DataFrame。

    参数:
        end_block: 结束区块（包含）。
        block_span: 覆盖区块跨度。
        chunk_size: RPC 分块查询尺寸。

    返回:
        pd.DataFrame: 标准化事件数据表。
    """
...
def render_overview_tab(df: pd.DataFrame, target_address: str) -> None:
    ...
    top_receivers = (
        ...
        .sum()
        .rename(columns={"value_usdc": "received_usdc"})
        .sort_values("received_usdc", ascending=False)
        .head(10)
    )
    if not top_receivers.empty:
        fig = px.bar(top_receivers, x="to", y="received_usdc", title="Top10 接收地址（按 USDC）")
...
def main() -> None:
    """Dashboard 主入口。"""
    st.set_page_config(page_title="Week4 USDC Dashboard", layout="wide")
    st.title("Week4 USDC Whale Dashboard（实时）")
    st.caption("数据流：fetcher -> parser -> analyzer -> dashboard")
    ...
    with st.spinner("正在拉取并解析实时 USDC 日志..."):
        df = load_realtime_data(end_block=end_block, block_span=int(block_span), chunk_size=int(chunk_size))
    if df.empty:
        st.warning("未获取到日志，请增加覆盖区块数后重试。")
        st.stop()

    tab1, tab2, tab3, tab4 = st.tabs(["概览", "事件流", "时间序列", "地址分析"])
    with tab1:
